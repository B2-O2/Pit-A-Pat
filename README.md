...
- Trouble  
  : 첫 화면에서 나타나는 배경 이미지 위에 부드럽게 본문이 올라오는 방식을 javascript 없이 css만으로 해결해야하는 문제가 있었다. 그리고 배경 이미지가 데스크탑, 태블릿, 모바일화면에서 모두 같은 시작위치에서 시작하면 모바일에선 배경 이미지의 끝부분만 보이는 문제가 있었다.

<br/>

- Shooting  
  : javascript를 사용하는 대신 배경이미지와 커버의 position을 fixed로 맞춘뒤 본문의 z-index를 배경이미지보다 높게 설정하여 구현하는 것으로 해결하였다. 또한 배경 이미지의 위치도 반응형 모니터에 맞게 위치를 각각 수정해줌으로써 해결하였다.
